...
func Help(param *model.TriggerParameter) bool {
	return ut.String().In(strings.TrimSpace(param.WrapperParam.EngineParam.GetTextMessage()), "ヘルプ", "帮助", "help", "?", "？", "")
}

func AISetting(param *model.TriggerParameter) bool {
	return ut.String().StartWith(param.WrapperParam.EngineParam.GetTextMessage(), "设定", "群角色设定")
}

func Health(param *model.TriggerParameter) bool {
	return ut.String().In(strings.TrimSpace(param.WrapperParam.EngineParam.GetTextMessage()), "ping")
}

func OffWorkTimeAnnounce(param *model.TriggerParameter) bool {
	return ut.String().In(strings.TrimSpace(param.WrapperParam.EngineParam.GetTextMessage()), "报时", "11")
}

func HolidayAnnounce(param *model.TriggerParameter) bool {
	return ut.String().In(param.WrapperParam.EngineParam.GetTextMessage(), "假期", "假期倒计时")
}

func RankOfGroupMsg(param *model.TriggerParameter) bool {
	text := strings.TrimSpace(param.WrapperParam.EngineParam.GetTextMessage())
	return text == "排名"
}
...
func SmartReply(param *model.TriggerParameter) bool {
	return ut.String().Contains(param.WrapperParam.EngineParam.GetTextMessage(), "人生", "不想上班", "好累", "工作")
}
...
func ExecSQL(param *model.TriggerParameter) bool {
	return ut.String().StartWith(param.WrapperParam.EngineParam.GetTextMessage(), "-sql ")
}

func CharacterPortrait(param *model.TriggerParameter) bool {
	return ut.String().In(param.WrapperParam.EngineParam.GetTextMessage(), "人格分析")
}
