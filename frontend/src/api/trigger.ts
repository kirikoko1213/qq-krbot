...
export const getTriggerList = () => {
  return request.get<DynamicTrigger[]>('/triggers/list');
};

// 保存动态触发器
export const saveTrigger = (data: DynamicTrigger) => {
  return request.post('/triggers/save', data);
};

// 删除动态触发器
export const deleteTrigger = (id: number) => {
  return request.post('/triggers/delete', { id });
};

// 获取单个动态触发器
export const getTrigger = (id: number) => {
  return request.get<DynamicTrigger>(`/triggers/${id}`);
};

// 获取函数列表
export const getFunctions = () => {
  return request.get<string[]>('/triggers/get-functions');
};

// 上移触发器
export const moveTriggerUp = (id: number) => {
  return request.post('/triggers/move-up', { id });
};

// 下移触发器
export const moveTriggerDown = (id: number) => {
  return request.post('/triggers/move-down', { id });
};
